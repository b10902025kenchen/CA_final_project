...
void BinGrid::initBin2D()
{
    ...
    for(int i = 0 ; i < _pModules.size() ; i++)
    {
        ...
        maximal_weight = max(maximal_weight, _pModules[i]->charge_density());
    }
    ...
    for (unsigned i = 0; i < _pModules.size(); i++) {
        ...
        for (int j = l_idx_x; j <= u_idx_x; j++) {
            for (int k = l_idx_y; k <= u_idx_y; k++) {
                Bin &bin = _Bin2D[j][k];
                bin.ovlp_area += ovlp_area(star, bin)*star.charge_density() / maximal_weight;
            }
        }
    }
    ...
    */
    ...
    for(int i = 0 ; i < _pModules.size() ; i++)
    {
        ...
        for(int k = bin_idx.first.first ; k <= bin_idx.second.first ; k++)
        {
            ...
            {
                double score_fac = _pModules[i]->charge_density() / maximal_weight;
    ...
    for(int i = 0 ; i < _pModules.size() ; i++)
        _field_cache[i] /= _pModules[i]->area()*_pModules[i]->charge_density() / maximal_weight;    //precondition
